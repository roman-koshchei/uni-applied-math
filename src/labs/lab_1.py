...
def problem_2_1_13():
    a = np.array([[1, 2], [-2, 1]])
    res = (
        np.linalg.matrix_power(a, 3) - 4 * np.linalg.matrix_power(a, 2) + a - np.eye(2)
    )
    print("2.1.13")
    print("x =", res)
...
def problem_2_4_13():
    a = np.array(
        [[-2, 2, 1, -1, -1], [3, 0, -1, 1, -1], [1, -1, 3, 2, 4], [2, 1, 3, 2, 2]]
    )
...
def problem_3_1_13():
    args = np.array([[3, 1, 2], [1, 3, 2], [2, 1, -1]])

    solutions = np.array([-2, 2, -1])
